...
import { Loader2,Youtube, Twitter, Instagram, Facebook, Linkedin } from "lucide-react";
...
export default function SocialMediaList({ onEdit, data }: any) {
  ...
  const handleDelete = async (id: any) => {
    if (window.confirm("Are you sure you want to delete this social media link?")) {
      ...
      try {
        await axiosInstance.delete(`/social-media/${id}`);
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-none bg-[#F7F9FC] dark:bg-dark-2 [&>th]:py-4 [&>th]:text-base [&>th]:text-dark [&>th]:dark:text-white">
            <TableHead className="min-w-[155px] xl:pl-7.5">Platform</TableHead>
        ...
        {!paginatedData || paginatedData.length === 0 ? (
            ...
            data.map((item: any) => {
              const Icon = platformIcons[item.platform]  // Get the icon for the platform
              return (
                <TableRow key={item._id} className="border-[#eee] dark:border-dark-3">
                  <TableCell className="min-w-[155px] xl:pl-7.5">
                    <div className="flex items-center gap-2">
                      {Icon && <Icon className="h-5 w-5" />} {/* Render the icon */}
                  ...
                  <TableCell>
                    <p className="text-dark dark:text-white">{item.url}</p>
                  </TableCell>
