...
export default function ReadyPropertyList({ data }: any) {
  ...
  const handleDelete = async (id: any) => {
    if (window.confirm("Are you sure you want to delete this Ready property?")) {
      setIsDeleting(true);
      setDeletingId(id);

      try {
        await axiosInstance.delete(`/ready-property/${id}`);
        toast.success("Ready property deleted successfully!");
        router.refresh();
      } catch (error) {
        console.error("Error deleting Ready property:", error);
        toast.error("Failed to delete Ready property.");
      } finally {
        setIsDeleting(false);
        setDeletingId(null);
      }
  ...
  const handleEdit = (id: any) => {
    router.push(`/admin/ready-property/edit-ready-property/${id}`);
  };
  ...
  return (
    ...
    <SearchControls 
      search={search}
      setSearch={setSearch}
      placeholder="Search Ready Properties..."
    /> 
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-none bg-[#F7F9FC] dark:bg-dark-2 [&>th]:py-4 [&>th]:text-base [&>th]:text-dark [&>th]:dark:text-white">
            ...
            <TableHead>Prperty type</TableHead>
              ...
              >
                ...
                <TableCell className="min-w-[155px] xl:pl-7.5">
                  <div className="flex items-center gap-3">
                    <Image
                      src={`${item.sliderImages[0]}` || "/images/placeholder.jpg"}
                ...
                <TableCell>
                  <p className="text-dark dark:text-white">{item.propertyStatus}</p>
                </TableCell>

                <TableCell>
                  <p className="text-dark dark:text-white">
                    {item.propertyType.title}
                  </p>
                </TableCell>
