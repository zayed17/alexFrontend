...
export default function ClientRequestList({ data }: any) {
  ...
  const handleDelete = async (id: any) => {
    if (window.confirm("Are you sure you want to delete this client request?")) {
      ...
      try {
        await axiosInstance.delete(`/client-request/${id}`);
        toast.success("Contact deleted successfully!");
        router.refresh();
      } catch (error) {
        console.error("Error deleting client request:", error);
        toast.error("Failed to delete client request.");
      } finally {
        setIsDeleting(false);
        setDeletingId(null);
      }
  ...
  return (
    <div className="space-y-6">
      <SearchControls
        search={search}
        setSearch={setSearch}
        placeholder="Search client-request datas..."
      />
